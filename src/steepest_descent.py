...
def steepest_descent(f, grad, x0, max_it, plot=False):
    '''
    DESCRIPTION
    ...
    while np.linalg.norm(gradfx) > 1e-5:  # while gradient is positive
        if it > max_it:
            ...
            break
        it += 1
        p = -gradfx  # search direction (Newton Method)
...
# tests 
# x_opt = steepest_descent(rosenbrock2, grad_rosenbrock2, [-1.2, 1], 100, plot=True)
# x_opt = steepest_descent(sphere, grad_sphere, random_point(2), 100, plot=True)



start_at = random_point(5)
x_opt = steepest_descent(obj_func, gradient_obj_func, start_at, 100000)
